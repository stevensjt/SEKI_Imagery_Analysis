...
trees <- #For comparison, both years have trees >3 inches (7.6 cm)
  trees[-which(between(trees$DBH,0.1,7.6)),]
...
trees_summ <- 
  ...
  summarise(n_trees = sum(Count),
            ba = sum(pi*((DBH/2)^2)*0.0001*Count), #in m2
...
trees_summ_15.2 <- #Trees > 15.2 cm (6 in)
  trees %>%
  group_by(Year, Subplot) %>%
  summarise(n_trees = sum(Count[DBH>15.2]),
            ba = sum(pi*((DBH[DBH>15.2]/2)^2)*0.0001*Count[DBH>15.2]) #in m2
  ) 
...
trees_summ_61.0 <- #Trees > 61.0 cm (24 in)
  ...
  summarise(n_trees = sum(Count[DBH>61]),
            ba = sum(pi*((DBH[DBH>61]/2)^2)*0.0001*Count[DBH>61]) #in m2
  ) 
...
subplots_ba_spp <-
  subplots %>%
  gather(Species,ba_spp,c(ABCO,ABMA,PICO,PIJE))
...
p2a<-
  ...
  labs(x = "times burned", y = "density (trees/ha; > 7.6 cm)", fill = "year") +
  theme_bw() +
...
ggplot(subplots,
  ...
  stat_summary(fun.data = "mean_se", geom = "errorbar",
               position = position_dodge(width=0.95), width = 0.5) +
  scale_fill_manual(values = c("1970" = "#fdc086", "2017" = "#386cb0"))+
  labs(x = "times burned", y = "basal area (m2/ha; trees > 7.6 cm)", fill = "year") +
  theme_bw() + 
...
p2c<-
  ...
  stat_summary(fun.data = "mean_se", geom = "errorbar",
               position = position_dodge(width=0.95), width = 0.5) +
  scale_fill_manual(values = c("1970" = "#fdc086", "2017" = "#386cb0"))+
  labs(x = "times burned", y = "density (trees/ha; > 15.2 cm)", fill = "year") +
  theme_bw()+
  theme(legend.position = "none")
p2d<-
  ggplot(subplots,
  ...
  stat_summary(fun.data = "mean_se", geom = "errorbar",
               position = position_dodge(width=0.95), width = 0.5) +
  scale_fill_manual(values = c("1970" = "#fdc086", "2017" = "#386cb0"))+
  labs(x = "times burned", y = "basal area (m2/ha; > 15.2 cm)", fill = "year") +
  theme_bw()+
  theme(legend.position = "none")
p2e<-
  ggplot(subplots,
  ...
  stat_summary(fun.data = "mean_se", geom = "errorbar",
               position = position_dodge(width=0.95), width = 0.5) +
  scale_fill_manual(values = c("1970" = "#fdc086", "2017" = "#386cb0"))+
  labs(x = "times burned", y = "density (trees/ha; > 61.0 cm)", fill = "year") +
  theme_bw()+
  theme(legend.position = "none")
p2f<-
  ggplot(subplots,
  ...
  stat_summary(fun.data = "mean_se", geom = "errorbar",
               position = position_dodge(width=0.95), width = 0.5) +
  scale_fill_manual(values = c("1970" = "#fdc086", "2017" = "#386cb0"))+
  labs(x = "times burned", y = "basal area (m2/ha; > 61.0 cm)", fill = "year") +
  theme_bw()+
  theme(legend.position = "none")
p2g<-
  ggplot(subplots,
...
p2h<- 
#  ggplot(subplots_ba_spp,
#         aes(x = Burned, y = ba_spp, fill = factor(Year))) +
#  geom_bar( stat="summary",fun.y = "mean",
#           position = position_dodge(width=0.95)) +
...
ggplot(subplots_ba_spp, aes(x = factor(Year), y = ba_spp, fill = Species)) +
  ...
  labs(x = "year", y = "basal area (m2/ha; > 7.6 cm)", fill = "species:  ",
       subtitle = "times burned") +
  facet_wrap( ~ Burned)+
  theme_bw() +
...
pdf("./Figures/MS/Fig2b.pdf", width = 8, height = 8)
grid.arrange(p2a,p2b,p2h,p2c,p2d,p2g,p2e,p2f, ncol = 3)
dev.off()
...
perims2 <- spTransform(perims, CRSobj = crs(r73scb)) #reproject perimeters
#plot(perims2, add=T) #Confirm alignment
